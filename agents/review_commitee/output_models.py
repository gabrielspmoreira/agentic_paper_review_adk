...
class MetaReview(BaseModel):
    """The final output from the meta-reviewer agent."""
    review_summary: str = Field(description="A summary of the consensus and disagreements among the reviewers.")
    major_weaknesses: List[str] = Field(description="A consolidated list of the most important strengths.")
    consolidated_cons: List[str] = Field(description="A consolidated list of the most critical weaknesses.")
    final_recommendation: Literal["Strong Accept", "Weak Accept", "Borderline", "Weak Reject", "Reject"] = Field(
        description="The final recommendation for the paper based on all reviews."
    )
    guidance_for_authors: str = Field(description="A concluding message to the authors guiding them on potential revisions.")